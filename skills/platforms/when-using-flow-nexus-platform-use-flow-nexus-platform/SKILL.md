...
description: Routing guardrail: enforce Flow Nexus platform skill when that platform is requested.
...
## Purpose
Ensure platform-specific work is handled by the Flow Nexus platform skill with correct SOP and safety gates.

## Trigger Conditions
- **Use this skill when:** User explicitly requests Flow Nexus platform usage or deployment guidance.
- **Reroute when:** If request is generic, use platform skill to select appropriate platform.
...
- MCP tagging for runs: `WHO=when-using-flow-nexus-platform-use-flow-nexus-platform-{session}`, `WHY=skill-execution`, namespace `skills/platforms/when-using-flow-nexus-platform-use-flow-nexus-platform/{project}`.
...
## Platform Playbook
- **Workflow patterns:**
  - Detect Flow Nexus platform intents and redirect
  - Restate guardrails and required artifacts for platform execution
  - Confirm user consent and proceed with flow-nexus-platform SOP
- **Anti-patterns to avoid:** Handling platform work without redirection, Dropping Flow Nexus governance requirements, Providing non-English or non-SOP responses
- **Example executions:**
  - Route 'deploy on Flow Nexus' to flow-nexus-platform skill
  - Map feature requests to platform topology and governance
